...
simulated dataset of 3' UTRs. Used to produce the 'simulated' portion of
observed vs. expected turnover events. Does this for every type of seed
conversion event
'''
...
import cPickle
import sys
data_folder, seed_types, start, end=sys.argv[1:]
...
for replicate in range(start, end):
	...
	gene_dict=cPickle.load(open(data_folder+'simulations/formatted_sim_wrong_anc_dict'+str(replicate), 'rb'))
	...
	cPickle.dump(master_counts, open(data_folder+'simulations/everything_simulated_counts'+str(replicate), 'wb'), -1)
