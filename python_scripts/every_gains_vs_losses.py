import custom
import sys
data_folder, seed_comparison=sys.argv[1:]
...
#	plt.grid(True, linestyle='-')
	...
	plt.savefig(data_folder+first_label+'_vs_'+second_label+'_'+seed_comparison+'.pdf', bbox_inches='tight')
	plt.clf()

gains=sorted([line.strip().split('\t') for line in open(data_folder+'mir_ranks_gain_'+seed_comparison)])
losses=sorted([line.strip().split('\t') for line in open(data_folder+'mir_ranks_loss_'+seed_comparison)])
output_file=open(data_folder+'gains_vs_losses_raw_data_'+seed_comparison, 'w')
