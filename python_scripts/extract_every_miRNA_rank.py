...
deviations every 8mer falls relative to mean of simulated values. Outputs
for every seed comparison
'''
import sys
import custom
data_folder, seed_comparison, mirfile=sys.argv[1:]
real_mirs={line.strip().split('\t')[0]:line.strip().split('\t')[1] for line in open(mirfile)}
stat_list=['gain', 'loss', 'sum']

for stat in stat_list:
	rank_list=[]
	output_file=open(data_folder+'mir_ranks_'+stat+'_'+seed_comparison, 'w')
	p_value_file=open(data_folder+'p_values_'+stat+'_'+seed_comparison, 'w')
	all_8mers=[line.strip().split('\t') for line in open(data_folder+'real_'+stat+'_'+seed_comparison)]
