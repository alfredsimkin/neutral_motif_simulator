'''
as a thought experiment (conducted on 11-11-19) what would this graph look like
in log2 space?
'''
...
def scatter_it(first, second, first_label, second_label, lims=False, faded=True):
	...
	'''
	first=[math.log(number, 2) for number in first]
	second=[math.log(number, 2) for number in second]
	import scipy.stats
	import matplotlib as mpl
	mpl.use('Agg') #comment this in if using a nongraphical interface, comment out to enable graphical interface
	import matplotlib.pyplot as plt
	import numpy
	plt.plot((0, math.log(65536, 2)), ((math.log(3277, 2)), (math.log(3277, 2))), 'b', linestyle='dashed')
	plt.plot(((math.log(3277, 2)), (math.log(3277, 2))), (0, math.log(65536, 2)), 'b', linestyle='dashed')
...
#	plt.grid(True, linestyle='-')
	...
	plt.savefig(data_folder+first_label+'_vs_'+second_label+'log2.png', bbox_inches='tight')
	plt.clf()
...
output_file=open(data_folder+'gains_vs_losses_raw_data_log2', 'w')
...
scatter_it(gains, losses, 'miRNA_gain_full_rank', 'miRNA_loss_full_rank_test', faded=False)
