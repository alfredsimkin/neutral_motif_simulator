...
change_dict=cPickle.load(open(data_folder+'real_5mer_counts', 'rb'))
...
def create_lists(change_dict):
	...
	for ancestor in change_dict:
		for descendant in change_dict[ancestor]:
			for anc_5mer in change_dict[ancestor][descendant]:
				descendant_list=make_pair(change_dict[ancestor][descendant][anc_5mer])
				list_dict[ancestor][descendant][anc_5mer]=descendant_list
	return list_dict
...
def simulate_descendant(anc_seq, desc_seq, ancestor, descendant):
	...
	for bp_number, bp in enumerate(sim_desc[:-2]):
		anc_5mer=sim_desc[bp_number:bp_number+5]
		if anc_5mer in list_dict[ancestor][descendant]:
			descendant_list=list_dict[ancestor][descendant][anc_5mer]
			desc_5mer=choice(descendant_list)
			sim_desc=sim_desc[:bp_number]+desc_5mer+sim_desc[bp_number+5:]
	return sim_desc
