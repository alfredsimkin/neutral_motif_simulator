...
change_dict=cPickle.load(open(data_folder+'real_3mer_counts', 'rb'))
...
def create_lists(change_dict):
	...
	for ancestor in change_dict:
		for descendant in change_dict[ancestor]:
			for anc_3mer in change_dict[ancestor][descendant]:
				descendant_list=make_pair(change_dict[ancestor][descendant][anc_3mer])
				list_dict[ancestor][descendant][anc_3mer]=descendant_list
	return list_dict
...
def simulate_descendant(anc_seq, desc_seq, ancestor, descendant):
	...
	for bp_number, bp in enumerate(sim_desc[:-2]):
		anc_3mer=sim_desc[bp_number:bp_number+3]
		if anc_3mer in list_dict[ancestor][descendant]:
			descendant_list=list_dict[ancestor][descendant][anc_3mer]
			desc_3mer=choice(descendant_list)
			sim_desc=sim_desc[:bp_number]+desc_3mer+sim_desc[bp_number+3:]
	return sim_desc
